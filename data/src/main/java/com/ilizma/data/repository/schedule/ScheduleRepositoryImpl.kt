...
class ScheduleRepositoryImpl @Inject constructor(
    ...
    ): Completable = scheduleRemoteDataSource.getSchedule()
        .doOnSuccess { scheduleList ->
            scheduleLocalDataSource.saveSchedule(scheduleList)
        }
        .ignoreElement()

    override fun getScheduleFromLocal(
        day: Int
    ): Single<List<Schedule>> = scheduleLocalDataSource.getSchedule()
        .map { allScheduleList ->
            allScheduleList.filter { it.day == day.toString() }
        }

    override fun isScheduleFromLocal(
    ): Completable = scheduleLocalDataSource.getSchedule()
        .ignoreElement()

    override fun nuke(
    ): Completable = scheduleLocalDataSource.nuke()
